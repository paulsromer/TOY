...
K40 = 1.3e-3.*mM.*(T./300).^-3.5.*exp(-11000./T) ;
...
i=i+1;
Knames{i} = 'KMT18';
krx(:,i) = 9.5e-39.*.21.*mM.*exp(5270./T)./(1+7.5e-29.*.21.*mM.*exp(5610./T)) ;

i=i+1;
Knames{i} = 'KBPPN'; %added by mmarvin 20141107
KPPN0 = 1.7e-03.*exp(-11280./T).*mM ;
...
%% accumulate
m = ~cellfun(@isempty,Knames);
Knames = Knames(m);
krx = krx(:,m);
K = struct;
for i=1:length(Knames)
    K.(Knames{i}) = krx(:,i);
    assignin('caller',Knames{i},krx(:,i));
